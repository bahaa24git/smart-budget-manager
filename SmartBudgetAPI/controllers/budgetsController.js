const { getAllBudgets, addBudget, updateBudgetBalance } = require('../models/budgetsModel');
...
exports.createBudget = async (req, res) => {
  ...
  try {
    ...
    const walletCheck = await pool.request()
      ...
      .query('SELECT * FROM UserWallets WHERE WalletID = @walletId AND UserID = @userId');

    if (walletCheck.recordset.length === 0) {
      return res.status(403).json({ message: 'This wallet does not belong to the authenticated user.' });
    }
